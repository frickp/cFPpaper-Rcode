...
par(font.lab=2)
boxplot(nl2 ~ Time.day, data=D.CHX500[D.CHX500$Time.day>-1,], notch=TRUE, xlab="Time in drug(d)", ylab="Colony Doublings", 
		border='grey20',ylim=c(-1,2.2))
n	<- length(unique(D.CHX500$id))
text(y=2,x=6,paste("n = ",n,sep=""))

#lo<-unique(cfp[cfp$nl2.72<0.7&cfp$CHX==500&cfp$Time.day==10&cfp$Erl==0,]$id)
#hi<-unique(cfp[cfp$nl2.72>1.7&cfp$CHX==500&cfp$Erl==0,]$id)
...
D.slope		<- subset(slopes,Erl==0&CHX==0)
...
col.ID	<-	unique(D.CHX500$id)
plot.try	<-	function(data,col.num)
{
	x		<-	subset(D.CHX500,id==col.ID[col.num])
	dev.new(width=3,height=4)
	plot(x$Time.day,x$nl2,ylim=c(-0.5,2))
} 
